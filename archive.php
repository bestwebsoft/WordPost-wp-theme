 ...
 */
get_header(); ?>
	<?php get_sidebar(); $count_posts = 1; ?>
	<div id="wordpost_content"><a id="top"></a>
		<div class="post">
			<h2 class="entry-title">
				<?php if ( is_day() ) {
					printf( __( 'Daily Archives: <span>%s</span>', 'wordpost' ), get_the_date() ); 
				} elseif ( is_month() ) {
					printf( __( 'Monthly Archives: <span>%s</span>', 'wordpost' ), get_the_date( 'F Y' ) );
				} elseif ( is_year() ) {
					printf( __( 'Yearly Archives: <span>%s</span>', 'wordpost' ), get_the_date( 'Y' ) );
				} else {
					printf( __( 'Archive of the tags: %s', 'wordpost' ), '<span>' . single_tag_title('', false) . '</span>' ); 
				} ?>
			</h2>
